...
- (100 points total) [Lab 8.1: Intro to basic assembly](#lab-1-intro-to-basic-assembly)
  ...
  - (50 points) [4. Assembly instruction: memory](#4-assembly-instruction-memory)
    ...
    - (15 Points) [4.4 Endianness: which side are you with?](#44-endianness-which-side-are-you-with)
...
> Hint#1: Check out `ldrb` and `strb` to read and write one byte from and to an address within a register. Also noted they won't load byte to/store byte from a 32-bit register like `x10`; instead, try using the register in 32-bit mode by using the name `w0`. 
...
> Note: MSB refers to most significant byte and LSB refers to least significant byte, check [3.1](#31-taking-one-byte-from-a-32-bit-word) for more explanation on them.
